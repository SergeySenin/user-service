...
### Модель данных
- **Liquibase-миграции.** Базовый changeset `user_V001__insert_users.sql` создаёт две таблицы:
  справочник стран `countries` и таблицу пользователей `users`, где хранится ссылка на страну проживания.
  В `users` лежат пути к исходным и преобразованным версиям аватаров, поэтому отдельной таблицы аватаров нет.
