...
### Модель данных
- **Liquibase-миграции.** Базовый changeset `001-users-schema-and-demo-data.sql` создаёт две таблицы:
  справочник стран `countries` и таблицу пользователей `users`, в которой хранится ссылка на страну проживания.
  В `users` находятся пути к исходным и преобразованным версиям аватаров, поэтому отдельной таблицы аватаров нет.
  Контекстно-зависимые TRUNCATE/INSERT выполняются только вне `prod`, и боевое окружение стартует с пустыми таблицами.
- **Учётные данные.** Поле пароля (`password`) отсутствует в таблице пользователей:
Keycloak остаётся единственным источником аутентификации, а сервис хранит только профильные атрибуты.
...
- После запуска доступна только административная консоль, поэтому конфигурацию нужно применить самостоятельно.
  Создайте realm `users`, публичный клиент `postman` (Direct Access Grants) и клиент-аудиторию `user-service`,
  добавляющую значение в claim `aud`. Пользователи `test_user` (роль `USER`) и `admin_user` (роль `ADMIN`)
  также заводятся вручную или импортируются из подготовленного экспорта — docker-compose их не загружает.
- Профиль `local` в `application-local.yaml` содержит базовые значения
...
### Ошибки безопасности
- 401 (`ErrorCode.UNAUTHORIZED`, `USR-4000`) — возвращается
  `JsonErrorAuthenticationEntryPoint`, если токен отсутствует или недействителен.
- 403 (`ErrorCode.ACCESS_DENIED`, `USR-4001`) — возвращается
